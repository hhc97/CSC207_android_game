...
public class Monster extends Character {
    ...
    private boolean isGetHit(){// Check is the monster is get hit by Hero or not
        int[] xRange = Level1Manager.player.getAttackRangeX();// get the x coordinate range of AttackRange
        int[] yRange = Level1Manager.player.getAttackRangeY();// get y coordinate range of AttackRange
        if ((x<=xRange[0] && xRange[0]<=(x+ WIDTH))||(x<=xRange[1] && xRange[1]<=(x+ WIDTH))){
            if((y<=yRange[0] && yRange[0]<=(y+ HEIGHT))||(y<=yRange[1] && yRange[1]<=(y+HEIGHT))){
                return true;
            }
        }
        return false;
